...
int hash_insert(hash_tbl * tbl, int val) {
  int loc, counter = 0;
  for(loc = hash(val); (tbl->tbl[loc] > EMPTY) && (counter < TBL_SZ); ++counter, loc = (loc + counter*counter) % TBL_SZ);
...
int hash_remove(hash_tbl * tbl, int val) {
  int loc, counter = 0;
  for(loc = hash(val); (tbl->tbl[loc] != val) && (tbl->tbl[loc] != EMPTY) && (counter < TBL_SZ); ++counter, loc = (loc + counter*counter) % TBL_SZ);
